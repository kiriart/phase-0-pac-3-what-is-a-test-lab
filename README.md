...
then run `npm install` followed by running a second command `npm test`. The 'npm install' command first installs the lab's
dependencies, then we run 'npm test' to run our tests and show the results.
...
> `npm install` is the command that installs the
> dependencies.
